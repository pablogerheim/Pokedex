...
function Corpo({
    ...
    function imprime({ color, id, image, titulo, types },i) {
        function bk_p() {
            color = dataProp[i].color === "white" ? "rgb(237, 224, 81)" : dataProp[i].color
        ...
        return (
            <div key={id} style={{ backgroundColor: bk_div() }} className="principal__cartao">
                ...
                <div className="principal__cartao__conteudo">
                    ...
                    <div className="principal__cartao__caracteristica">
                        {types.map(type => { return <p key={v4()} style={{ backgroundColor: bk_p() }} className="principal__cartao__caracteristica__cada">{type.type.name[0].toUpperCase() + type.type.name.substr(1)}</p> })}
                    </div>
