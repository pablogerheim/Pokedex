import axios from 'axios'

async function pokeAPIpage(offset = 0) {
    let pokedexArr = []
    let URLs = []
    let data = await axios.get(`https://pokeapi.co/api/v2/pokemon?offset=${offset}&limit=12`)
    data.data.results.map(item => URLs.push(item.url))

    let pokeInfo = await Promise.all(URLs.map(url => axios.get(url).then(res => res.data)))
    let pokeColor = await Promise.all(URLs.map(url => axios.get(url.replace("pokemon", "pokemon-species")).then(res => res.data.color.name)))

    pokeInfo.map((item, i) => {
        pokedexArr.push({
            "titulo": item.name,
            "id": item.order,
            "types": item.types,
            "image": item.sprites.other.dream_world.front_default,
            "color": pokeColor[i]
        })
    })
}
pokeAPIpage()

let pokeBaseSearch = []
async function pokeAPIbase() {
    let dataLenthg = await axios.get(`https://pokeapi.co/api/v2/pokemon?limit=1`).then(res => res.data.count)
    pokeBaseSearch = await axios.get(`https://pokeapi.co/api/v2/pokemon?limit=${dataLenthg}`).then(res => res.data.results)
}
pokeAPIbase()


async function pokeAPIsearch(serach = '') {
    console.log(pokeBaseSearch)
    let filtred = pokeBaseSearch.filter(({ name }) => name.toLocaleLowerCase().includes(serach.toLocaleLowerCase()))
    console.log(filtred)
    let URLs = []
    filtred.map((item, i) => { if (i >= 12) { console.log(item.name) } else URLs.push(item.url) })
    let pokedexArr = []
    let pokeInfo = await Promise.all(URLs.map(url => axios.get(url).then(res => res.data)))
    let pokeColor = await Promise.all(URLs.map(url => axios.get(url.replace("pokemon", "pokemon-species")).then(res => res.data.color.name)))

    pokeInfo.map((item, i) => {
        pokedexArr.push({
            "titulo": item.name,
            "id": item.order,
            "types": item.types,
            "image": item.sprites.other.dream_world.front_default,
            "color": pokeColor[i]
        })
    })

}



function Rapi() {

    const data = {
        pokedex: [{
            id: "#001",
            titulo: "Pulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#002",
            titulo: "Aulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#003",
            titulo: "Dulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#004",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#005",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#006",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#007",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#008",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#009",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#010",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#011",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#012",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#013",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#014",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#015",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#016",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#017",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#018",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#019",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#020",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#021",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#022",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#023",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#024",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#025",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#026",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#027",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#028",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#029",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#030",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#031",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#032",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#033",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#034",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#035",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#036",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#101",
            titulo: "Pulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#102",
            titulo: "Aulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#103",
            titulo: "Dulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#104",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#105",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#106",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#107",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#108",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#109",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#110",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#111",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#112",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#113",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#114",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#115",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#116",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#117",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#118",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#119",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#120",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#121",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#122",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#123",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#124",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#125",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#126",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#127",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#128",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#129",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#130",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#131",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#132",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#133",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#134",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#135",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#136",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#201",
            titulo: "Pulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#202",
            titulo: "Aulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#203",
            titulo: "Dulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#204",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#205",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#206",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#207",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#208",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#209",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#210",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#211",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#212",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#213",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#214",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#215",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#216",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#217",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#218",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#219",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#220",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#221",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#222",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#223",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#224",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#225",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#226",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#227",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#228",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#229",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#230",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#231",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#232",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#233",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#234",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#235",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#236",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#301",
            titulo: "Pulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#302",
            titulo: "Aulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#303",
            titulo: "Dulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#304",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#305",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#306",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#307",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#308",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#309",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#310",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#311",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#312",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#313",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#314",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#315",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#316",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#317",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#318",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#319",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#320",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#321",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#322",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#323",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#324",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#325",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#326",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#327",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#328",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#329",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#330",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#331",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#332",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#333",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#334",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#335",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#336",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#401",
            titulo: "Pulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#402",
            titulo: "Aulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#403",
            titulo: "Dulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#404",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#405",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#406",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#407",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#408",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#409",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#410",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#411",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#412",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#413",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#414",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#415",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#416",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#417",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#418",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#419",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#420",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#421",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#422",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#423",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#424",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#425",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#426",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#427",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#428",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#429",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#430",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#431",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#432",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#433",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#434",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#435",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#436",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#501",
            titulo: "Pulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#502",
            titulo: "Aulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#503",
            titulo: "Dulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#504",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#505",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#506",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#507",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#508",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#509",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#510",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#511",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#512",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#513",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#514",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#515",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#516",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#517",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#518",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#519",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#520",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#521",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#522",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#523",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#524",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#525",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#526",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#527",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#528",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#529",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#530",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#531",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#532",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#533",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#534",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#535",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        {
            id: "#536",
            titulo: "Bulbasaur",
            caracteristica1: "Grass",
            caracteristica2: "Poison",
            image: "../img/doguito.svg"
        },
        ]
    }
    return (data)
}
export { Rapi }